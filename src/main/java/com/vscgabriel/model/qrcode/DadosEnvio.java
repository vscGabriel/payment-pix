package com.vscgabriel.model.qrcode;
...
public record DadosEnvio(Key chavePIX, BigDecimal valor, String cidadeRemetente, String descricao) {
    ...
    public DadosEnvio(Key chavePIX, BigDecimal valor, String cidadeRemetente) {
        this(chavePIX, valor, cidadeRemetente, "");
    }

    public DadosEnvio {
        if(requireNonNull(chavePIX.name()).isBlank())
            throw new IllegalArgumentException("O nome do destinatário é obrigatório.");
        var nomeDestinatario = chavePIX.name();
        var chaveDestinatario = chavePIX.key();
        if(chavePIX.key().length() > TAMANHO_NOME) {
